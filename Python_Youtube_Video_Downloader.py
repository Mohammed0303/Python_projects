...
root.title("Python Youtube video downloader application")
...
def download():
    ...
    try:
        ...
        root.update()
        YouTube(link.get()).streams.filter(progressive=True, file_extension='mp4').order_by('resolution').desc().first().download()
